use simplicial_topology::simplicial_complex::simplicial_complex::{Facet, Simplex, SimplicialComplex};
...
fn main() {
    let facets: Vec<Facet> = vec![Simplex::new((0..20).collect())];
    //let bdy: Vec<Facet> = facets[0].boundary();
    let sc:SimplicialComplex = SimplicialComplex::new(facets).k_skeleton(3);
    sc.print(); 
    let dim = sc.dimension();
    for i in 0..dim{
        println!("{}", i);
    }
